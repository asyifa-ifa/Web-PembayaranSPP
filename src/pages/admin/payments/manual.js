...
export default function ManualPayment() {
  ...
  // Multi-select: pakai Set of bill IDs
  ...
  useEffect(() => {
    ...
    if (billId) {
      setSelectedBillIds(new Set([Number(billId)]));
    }
  }, [router.query]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/students/${selectedStudent}/detail`)
      ...
      .then((data) => {
        ...
        // Jika ada billId dari query, pre-select
        const billIdParam = router.query.billId ? Number(router.query.billId) : null;
        if (billIdParam && unpaid.find((b) => b.id === billIdParam)) {
          setSelectedBillIds(new Set([billIdParam]));
          setForm((prev) => ({
            ...prev,
  ...
  // Hitung total dari tagihan yang dipilih
  const selectedBills = bills.filter((b) => selectedBillIds.has(b.id));
  const totalAmount = selectedBills.reduce((sum, b) => sum + b.amount, 0);

  // Ambil paymentType name unik dari tagihan terpilih
  const selectedTypeNames = [...new Set(selectedBills.map((b) => b.paymentType?.name).filter(Boolean))];

  const handleSubmit = async () => {
    if (!selectedStudent) return alert("Pilih santri dulu");
    if (selectedBillIds.size === 0) return alert("Pilih minimal 1 tagihan");
    ...
    try {
      ...
      alert(`✅ ${data.message || "Pembayaran berhasil dicatat & email terkirim!"}`);
  ...
  const isReady = selectedStudent && selectedBillIds.size > 0 && form.academicYear;
  ...
  return (
    <AdminLayout>
      <div className="page-wrapper">
        ...
        <div className="content-grid">
          <div className="form-card">
            ...
            {/* Section 2: Tagihan - MULTI SELECT */}
            {selectedStudent && (
              <div className="section">
                <div className="section-header">
                  ...
                  <div style={{ flex: 1 }}>
                    <div className="section-title">
                      ...
                      {selectedBillIds.size > 0 && (
                        <span className="selected-count-badge">{selectedBillIds.size} dipilih</span>
                      )}
                    </div>
                    <div className="section-sub">Klik tagihan untuk memilih — bisa pilih lebih dari satu</div>
                  </div>
                  {bills.length > 0 && (
                ...
                {bills.length === 0 ? (
                  ...
                  <>
                      ...
                      {bills.map((b) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() => handleBillToggle(b.id)}
                          >
                            <div className="bill-left">
                              <div className="bill-check">{isSelected ? "✓" : ""}</div>
                              <div>
                                <div className="bill-name">
                                  {b.paymentType.name}
                                  {b.month && (
                                    <span className="bill-month-badge">{b.month}</span>
                                  )}
                                </div>
                    ...
                    {/* Total bar muncul kalau ada yang dipilih */}
                    {selectedBillIds.size > 0 && (
                      <div className="total-bar">
                        <div className="total-bar-left">
                          <span className="total-bar-count">{selectedBillIds.size} tagihan dipilih</span>
                        </div>
                        <div className="total-bar-right">
                          <span className="total-bar-label">Total:</span>
                          <span className="total-bar-amount">Rp {formatRupiah(totalAmount)}</span>
                        </div>
                      </div>
                    )}
                  </>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  Konfirmasi {selectedBillIds.size > 0 ? `${selectedBillIds.size} Tagihan` : "Pembayaran"}
          ...
          <div className="summary-panel">
            <div className="summary-card">
              ...
              {/* Daftar tagihan terpilih */}
              {selectedBills.length > 0 ? (
                <>
                  <div style={{ marginBottom: 8 }}>
                    <span className="summary-label">Tagihan Dipilih ({selectedBills.length})</span>
                  </div>
                  {selectedBills.map((b) => (
                    <div key={b.id} className="summary-bill-row">
                      <span className="summary-bill-name">
                        {b.paymentType?.name}
                        {b.month ? ` (${b.month})` : ""}
                      </span>
                      <span className="summary-bill-amt">Rp {formatRupiah(b.amount)}</span>
                    </div>
                  ))}
                </>
              ) : (
                <div className="summary-row">
                  <span className="summary-label">Tagihan</span>
      ...
      <style jsx>{`
        ...
        .section-title { font-size: 15px; font-weight: 600; color: #14532d; display: flex; align-items: center; gap: 8px; }
        .section-sub { font-size: 12px; color: #9ca3af; margin-top: 1px; }
        .selected-count-badge { background: #16a34a; color: white; font-size: 11px; font-weight: 700; padding: 2px 8px; border-radius: 20px; }
        .select-all-btn { margin-left: auto; flex-shrink: 0; padding: 6px 12px; background: #f0fdf4; border: 1.5px solid #86efac; color: #15803d; border-radius: 8px; font-size: 12px; font-weight: 600; cursor: pointer; transition: all .15s; white-space: nowrap; }
        .select-all-btn:hover { background: #dcfce7; }
        ...
        .bill-item { display: flex; align-items: center; justify-content: space-between; padding: 12px 14px; border: 1.5px solid #e5e7eb; border-radius: 10px; cursor: pointer; transition: all .15s; background: white; user-select: none; }
        ...
        .bill-name { font-size: 13px; font-weight: 500; color: #111827; display: flex; align-items: center; gap: 6px; flex-wrap: wrap; }
        .bill-month-badge { margin-left: 4px; font-size: 11px; background: #dbeafe; color: #1d4ed8; padding: 1px 8px; border-radius: 20px; font-weight: 600; }
        ...
        /* Total bar */
        .total-bar { display: flex; align-items: center; justify-content: space-between; margin-top: 12px; padding: 12px 16px; background: linear-gradient(135deg, #14532d, #16a34a); border-radius: 10px; }
        .total-bar-left { display: flex; align-items: center; gap: 8px; }
        .total-bar-count { font-size: 13px; color: rgba(255,255,255,.85); font-weight: 500; }
        .total-bar-right { display: flex; align-items: center; gap: 8px; }
        .total-bar-label { font-size: 13px; color: rgba(255,255,255,.75); }
        .total-bar-amount { font-size: 16px; font-weight: 700; color: white; }
        /* Form fields */
        ...
        .summary-bill-row { display: flex; justify-content: space-between; align-items: center; gap: 8px; margin-bottom: 6px; }
        .summary-bill-name { font-size: 12px; color: #374151; flex: 1; }
        .summary-bill-amt { font-size: 12px; font-weight: 600; color: #dc2626; white-space: nowrap; }
        ...
        @media (max-width: 900px) { .content-grid { grid-template-columns: 1fr; } .summary-panel { position: static; } }
