...
// PALET WARNA (Diperbarui dengan Soft Terracotta Cerah nan Lembut untuk Kontras Pengeluaran)
const COLORS = {
  ...
  terracottaSoft: "#E0876A", // BARU: Warna Pengeluaran Cerah & Soft (Coral Muted)
};
...
function TooltipPengeluaran({ active, payload, label }) {
  ...
  return (
    <div style={tooltip.box}>
      <p style={tooltip.label}>{label}</p>
      {payload.map((p, i) => {
        // Sinkronisasi warna indikator teks di tooltip
        const textColor = p.name === "Pengeluaran" ? COLORS.terracottaSoft : p.color;
        return (
          <p key={i} style={{ ...tooltip.val, color: textColor, margin: "4px 0 0", fontSize: 13 }}>
      ...
      <div className="dash-root" style={{ display:"flex", flexDirection:"column", gap:24 }}>
        ...
        <div className="fu" style={{ animationDelay:"60ms", display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))", gap:16 }}>
          ...
          <StatCard
            ...
            accent={COLORS.terracottaSoft}
            iconColor={COLORS.terracottaSoft}
            icon={
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
          ...
          <StatCard
            ...
            subColor={selisih >= 0 ? COLORS.greenSoft : COLORS.terracottaSoft}
            icon={
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <>
              <div style={{ display:"flex", gap:32, marginBottom:20, borderBottom:`1px solid ${COLORS.border}`, paddingBottom:14, flexWrap:"wrap" }}>
                {[
                  ...
                  { label:"Bulan Pengeluaran Tertinggi", val: pengeluaranData.reduce((a,b)=>(b.pengeluaran||0)>(a.pengeluaran||0)?b:a,{}).bulan||"—", color: COLORS.terracottaSoft }, // Teks bulan tertinggi ikut berganti warna teracota
                ...
                <AreaChart data={pengeluaranData} margin={{ top:8, right:16, left:8, bottom:0 }}>
                  <defs>
                    <linearGradient id="gradExp" x1="0" y1="0" x2="0" y2="1">
                      {/* Gradasi bawah diubah ke Terracotta dengan opasitas tipis agar tetap soft */}
                      <stop offset="5%"  stopColor={COLORS.terracottaSoft} stopOpacity={0.25}/>
                      <stop offset="95%" stopColor={COLORS.terracottaSoft} stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  ...
                  <Area type="monotone" dataKey="pengeluaran" name="Pengeluaran" stroke={COLORS.terracottaSoft} strokeWidth={2.5} fill="url(#gradExp)" dot={{ r:4, fill: COLORS.terracottaSoft, strokeWidth:0 }} activeDot={{ r:6, strokeWidth:0 }}/>
                </AreaChart>
              </ResponsiveContainer>
            </>
          )}
            ...
            {keuanganData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={keuanganData} margin={{ top:8, right:16, left:8, bottom:0 }} barGap={6}>
                  ...
                  <Bar dataKey="pengeluaran" name="Pengeluaran" fill={COLORS.terracottaSoft} radius={[6,6,0,0]} maxBarSize={32} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
